...
def read_gencode_gene_type(annotation_file = 'data/gencode.v19.genes.v6p_model.patched_contigs.gtf.gz'):
    gene_type_dict = {}
    gene_name_ensg = {}
    gene_id_position = {}
    fp = gzip.open(annotation_file)
    for line in fp:
        if line[0] == '#':
            continue
        values = line.rstrip('\r\n').split('\t')
...
def read_disgent_database():
    disease_gene_dict = {}
    print 'reading DisGeNET'
    with open('data/DisGeNET_new.txt') as fp:
        for line in fp:
            values = line.rstrip().split('\t')
            gene = values[0]
            disease = values[1]
...
def read_gencode_mirna_annotation():
    ...
    gene_name_ensg = {}
    gene_id_position = {}
    fp = gzip.open('data/gencode.v19.genes.v7.patched_contigs.gtf.gz')
    for line in fp:
        if line[0] == '#':
            continue
        values = line.rstrip('\r\n').split('\t')
...
def read_gencode_mirna_annotation_new(annotation_file = 'data/gencode.v19.genes.v7.patched_contigs.gtf.gz'):
    gene_type_dict = {}
    gene_name_ensg = {}
    gene_id_position = {}
    fp = gzip.open(annotation_file)
...
def read_v10_v11_map(map_file = 'data/human.v10_v11.tsv.gz'):
    ensp_v10_v11 = {}
    with gzip.open(map_file) as fp:
        for line in fp:
            if line[0] == '#':
                continue
            values = line.rstrip().split()
            ensp_v10 = values[1]
            ensp_v11 = values[2]
            ensp_v10_v11[ensp_v11] = ensp_v10
    return ensp_v10_v11

def read_ensp_ensg_v11(map_file = 'data/9606.protein.aliases.v11.0.txt.gz'):
    ensp_ensg = {}
    ensp_v10_v11 = read_v10_v11_map()
    with gzip.open(map_file) as fp:
        for line in fp:
            values = line.rstrip().split()
            ensg = values[1]
            if 'ENSG' not in ensg:
                continue
            ensp = values[0].split('.')[-1]
            ensp_ensg[ensp] = ensg
    return ensp_ensg

def read_string_interaction(string_file = 'data/9606.protein.links.v10.txt.gz', cutoff = 500):
...
def read_ncrna_interaction(ncrna_file = 'data/9606.v1.combined.tsv.gz', cutoff = 0.20):
    ensp_ensg = read_ensp_ensg()
    interact_pair = {}
    genes = set()
    mirna_set = set()
...
def plot_dis_degree(gene_dis_dict, plot_title):
    counts = []
    for key in gene_dis_dict:
        counts.append(len(gene_dis_dict[key]))

    from collections import Counter
    c = Counter(counts)

    plt.bar(c.keys(), c.values())
    plt.xlabel('# of disease')
    plt.ylabel('Count')
    plt.title(plot_title)
    plt.show()

def read_expression_data_old(input_file = 'data/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct.gz', data = 3):
    ...
    annotation_file = 'data/gencode.v19.genes.v7.patched_contigs.gtf.gz'
    gene_type_dict, gene_name_ensg, gene_id_position = read_gencode_gene_type(annotation_file)

    whole_data, samples = read_gtex_expression(input_file, gene_type_dict)

    mirna_ensg_name = read_gencode_mirna_annotation_new(annotation_file)
    minum =0
    ...
    print all_mrna_len, 'have interaction', has_int
    ...
    for mirna, val in mirna_disease.iteritems():
        if mirna in mirna_ensg_name:
            ...
            new_dis11 = new_dis11 | val
            ddd_sum = ddd_sum + len(val)

    print 'all dis-mirna', ddd_sum, len(new_dis11)
    mirna_disease = {}
    ...
    return adj, torch.FloatTensor(features), torch.FloatTensor(labels), torch.LongTensor(np.array(idx_train)), \
           torch.LongTensor(np.array(idx_val)), torch.LongTensor(np.array(idx_test)), all_dis, new_mirna_disease, disease_miRNA_data, None
...
def read_expression_data(input_file='data/GTEx_Analysis_2016-01-15_v7_RNASeQCv1.1.8_gene_median_tpm.gct.gz', data = 3):
    ensg_ensp_map = read_ensp_ensg()
    disease_gene_dict, disease_name_map, whole_disease_gene_dict = read_DISEASE_database(confidence=2, ensg_ensp_map = ensg_ensp_map)
    disease_name_map, whole_disease_gene_dict = {}, {}

    #annotation_file = 'data/gencode.v19.genes.v6p_model.patched_contigs.gtf.gz'
    annotation_file = 'data/gencode.v19.genes.v7.patched_contigs.gtf.gz'
    gene_type_dict, gene_name_ensg, gene_id_position = read_gencode_gene_type(annotation_file)
   ...
   # whole_data, samples = read_gtex_expression(input_file, gene_type_dict)
    ...
    #plot_dis_degree(mrna_diseases, 'PCG')
    ...
    inter_ensg_no_dis = []
    for ens in all_genes:
        if ens in all_disease_ensg:
            continue
        if ens in whole_data:
            inter_ensg_no_dis.append(ens)


    all_mrna_len = len(all_disease_ensg)
    all_mrna_set = set(all_disease_ensg + inter_ensg_no_dis)
    print all_mrna_len
    ...
    disease_miRNA_data = list(filter_mirna_set)
    all_exports = all_disease_ensg + disease_miRNA_data
    label_len = len(all_exports)
    all_inter_genes = all_exports + inter_ensg_no_dis
    gene_len = len(all_inter_genes)
    adj = np.zeros((gene_len, gene_len))
    #all_export_sets = set(all_exports)
    #new interaction string_interaction
    for i in range(1, gene_len):
        for j in range(i):
            key = (all_inter_genes[i], all_inter_genes[j])
            if key in string_interaction or key in new_rain_interaction:
                adj[i, j] = 1
                adj[j, i] = 1
    ...
    disease_miRNA_set = set(disease_miRNA_data)
    mirna_disease = read_mirna_disease()
    #plot_dis_degree(mirna_disease, 'microRNA')
    new_mirna_disease = {}
    for mirna, val in mirna_disease.iteritems():
        if mirna in mirna_ensg_name:
            mirna_ensg = mirna_ensg_name[mirna]
    ...
    all_index_all = range(gene_len)
    all_index = all_index_all[:label_len]
    idx_train_val = all_index[:all_mrna_len]
    random.shuffle(idx_train_val)
    #keep 20% as validation
    train_num = int(0.9*all_mrna_len)
    ...
    idx_test = all_index[all_mrna_len:] + all_index_all[label_len:]
    mirna_len = label_len - all_mrna_len
    ...
    return adj, torch.FloatTensor(features), torch.FloatTensor(labels), torch.LongTensor(np.array(idx_train)), \
           torch.LongTensor(np.array(idx_val)), torch.LongTensor(np.array(idx_test)), all_dis, new_mirna_disease, disease_miRNA_data, mirna_len, inter_ensg_no_dis
...
def train(epoch, model, optimizer, features, adj, idx_train, labels, idx_val, criteria = None, hierar_penalty = 0.01,
          is_hierar = True, is_multi =True, hierar_relations = None):
    t = time.time()
    linear_paras = model.gc2.weight
    #linear_paras = model.out_att.W
    model.train()
    optimizer.zero_grad()
    ...
    if is_hierar:
        print 'hierarchial'
        used_argvs = (hierar_penalty, linear_paras, hierar_relations)
        loss_train = criteria(output[idx_train], labels[idx_train], is_hierar, is_multi, *used_argvs)
    else:
    ...
    #pdb.set_trace()
    if is_hierar:
        loss_val = criteria(output[idx_val], labels[idx_val], is_hierar, is_multi, *used_argvs)
    else:
...
def plot_roc_curve_lncRNA(labels, probality, legend_text):
    ...
    precision, recall, thresholds = metrics.precision_recall_curve(labels, probality)
    auprc = auc(recall, precision)

    y_pred_all = [1 if x >= 0.5 else 0 for x in probality]

    rects1 = plt.plot(fpr, tpr, label=legend_text + '(AUC=%6.3f) ' % roc_auc)
    return roc_auc, auprc


def plot_prroc_curve_lncRNA(labels, probality, legend_text):
    # fpr2, tpr2, thresholds = roc_curve(labels, pred_y)
    fpr, tpr, thresholds = roc_curve(labels, probality)  # probas_[:, 1])
    roc_auc = auc(fpr, tpr)
    ...
    rects1 = plt.plot(recall, precision, label=legend_text + '(AUPRC=%6.3f) ' % auprc)
    return roc_auc, auprc
...
def test_obo_parser():
    ...
    fw = open('disease_doid.taxonomy', 'w')
    doid_terms = oboparser.terms
    #doid_list = doid_terms.keys()
    #pdb.set_trace()
    for doid_key, doid_val in doid_terms.iteritems():
        if len(doid_val['c']):
            fw.write(doid_key + '\t')
            fw.write('\t'.join(doid_val['c']))
            fw.write('\n')
            #for val in doid_val['c']:
            #    fw.write(val)
...
def get_disease_hier(all_dis):
    hierar_taxonomy = 'disease_doid.taxonomy'
    label_map = {}
    for ind, dis in enumerate(all_dis):
        label_map[dis] = ind

    hierar_relations = get_hierar_relations(hierar_taxonomy, label_map)
    return hierar_relations

def run_gcn(lr = 0.0001, num_neu = 3, weight_decay = 0.001, dropout = 0.8, hierar_penalty = 0.01):
    adj = []
    features = []
    ...
    for epoch in range(epochs):
        auc_train = train(epoch, model, optimizer, features, adj, idx_train, labels, idx_val, criterion, hierar_penalty= hierar_penalty,
                          hierar_relations = hierar_relations, is_hierar = is_hierar)
